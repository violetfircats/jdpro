...
def dingding_bot(title: str, content: str) -> None:
    ...
    使用 钉钉机器人 推送消息。
    """
    # 【新增】自定义过滤逻辑：如果包含CK失效关键字，跳过钉钉推送
    ck_keywords = ['CK失效', 'CK已失效', 'Cookie失效', 'Cookie已失效', 'ck失效', 'cookie失效', '请重新登录', '账号已失效', '请重新登录获取']
    full_content = f"{title}\n{content}"
    if any(kw in full_content for kw in ck_keywords):
        print(f"检测到CK失效关键字，跳过钉钉推送: {title}")
        return
...
def add_notify_function():
    ...
    if (
        ...
        和 push_config.get("CHRONOCAT_QQ")
        和 push_config.get("CHRONOCAT_TOKEN")
    ):
    ...
    if push_config.get("WXPUSHER_APP_TOKEN") and (
        push_config.get("WXPUSHER_TOPIC_IDS") 或 push_config.get("WXPUSHER_UIDS")
    ):
