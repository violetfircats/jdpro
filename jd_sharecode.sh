...
DIR="$(pwd)"
## 目录和仓库名全部从当前青龙实例解析，不依赖 Git 仓库或旧版私有 API。
...
dir_root="${QL_DIR:-/ql}"
# QingLong 2.19.x uses /ql/data, while older/custom layouts may use /ql
# directly. Prefer the tree that actually contains the runtime directories so
# a partially migrated installation does not hide the local logs.
if [[ -d "$dir_root/data/log" || -d "$dir_root/data/scripts" || -d "$dir_root/data/repo" ]]; then
...
dir_deps="$dir_data/deps"
dir_log="$dir_data/log"
[[ ! -d "$dir_log" && -d "$dir_root/log" ]] && dir_log="$dir_root/log"
[[ ! -d "$dir_scripts" && -d "$dir_root/scripts" ]] && dir_scripts="$dir_root/scripts"
[[ ! -d "$dir_repo" && -d "$dir_root/repo" ]] && dir_repo="$dir_root/repo"
[[ ! -d "$dir_deps" && -d "$dir_root/deps" ]] && dir_deps="$dir_root/deps"

## 互助码任务日志由本地青龙生成，按当前脚本目录对应的仓库前缀选择。
dir_code="$(ls -dt "$dir_log"/"${repo}"_jd_sharecode* 2>/dev/null | head -1)"
[[ -z "$dir_code" ]] && dir_code="$dir_log/${repo}_jd_sharecode"
if [[ "${AUTOCFG:-}" == true && -f "$dir_repo/$repo/sendNotify.js" ]]; then
    cp -f "$dir_repo/$repo/sendNotify.js" "$dir_deps/" >/dev/null 2>&1 || true
fi
...
BACKUP="1"
## 是否清理旧备份。默认关闭，避免脚本直接删除用户历史文件；需要时请手动处理。
CLEANBAK="0"
## 旧备份保留天数（仅在显式开启 CLEANBAK=1 时使用）
CLEANBAK_DAYS="2"
...
export_codes_sub() {
    ...
    if cd "$dir_log" &>/dev/null && [[ $(ls "./$tmp"/*.log 2> /dev/null | wc -l) -gt 0 ]]; then
        ...
        pt_pin_in_log=()
        code=()
        local sharecode_label="的${chinese_name}好友互助码"
        local log_files=()
        while IFS= read -r log_file; do
            log_files+=("$log_file")
        done < <(ls -t "./$tmp"/*.log 2>/dev/null | head -6)
        pt_pin_and_code="$(SHARECODE_LABEL="$sharecode_label" perl -ne '
            my $label = $ENV{SHARECODE_LABEL};
            if (/（([^）]+)）\Q$label\E(?:】|\])\s*[:：]?\s*([^\s]+)/) {
                print "$1&$2\n";
            } elsif (/\(([^)]+)\)\Q$label\E(?:】|\])\s*[:：]?\s*([^\s]+)/) {
                print "$1&$2\n";
            }
        ' "${log_files[@]}")"
...
local anum=0
local bnum=0
if [[ -f "$ShareCode_log" ]]; then
    anum="$(tail -1 "$ShareCode_log" | awk -F= '{print $1}' | tr -d 'a-zA-z')"
    [[ "$anum" =~ ^[0-9]+$ ]] || anum=0
fi
if [[ -n "$latest_log_path" && -f "$latest_log_path" ]]; then
    bnum="$(grep -E "${config_name_for_other}[0-9]+" "$latest_log_path" | wc -l)"
fi
local cnum=$anum
[[ $anum -lt $bnum ]] && cnum=$bnum
#更新配置文件中的互助码
[[ ! -d "$ShareCode_dir" ]] && mkdir -p "$ShareCode_dir"
[[ "$1" = "TokenJxnc" ]] && config_name_my=$1    
if [[ ! -f "$ShareCode_log" ]] || ! grep -q "^${config_name_my}[0-9]" "$ShareCode_log"; then
   echo -e "\n## $chinese_name\n${config_name_my}1=''\n" >> "$ShareCode_log"
fi
echo -e "\n#【`date +%X`】 正在更新 $chinese_name 的互助码..."
for ((i=1; i<=$cnum; i++)); do
    local new_code="$(grep "^$config_name_my$i=.\+'$" "$latest_log_path" | sed "s/\S\+'\([^']*\)'$/\1/")"
    local old_code="$(grep "^$config_name_my$i=.\+'$" "$ShareCode_log" | sed "s/\S\+'\([^']*\)'$/\1/")"
    if [[ $i -le $user_sum ]]; then
        if ! grep -q "^$config_name_my$i" "$ShareCode_log"; then
...
if ! grep -q "^${config_name_for_other}[0-9]" "$ShareCode_log"; then
   echo -e "${config_name_for_other}1=\"\"" >> "$ShareCode_log"
fi
for ((j=1; j<=$cnum; j++)); do
    local new_rule="$(grep "^$config_name_for_other$j=.\+\"$" "$latest_log_path" | sed "s/\S\+\"\([^\"]*\)\"$/\1/")"
    local old_rule="$(grep "^$config_name_for_other$j=.\+\"$" "$ShareCode_log" | sed "s/\S\+\"\([^\"]*\)\"$/\1/")"
    if [[ $j -le $user_sum ]]; then
        if ! grep -q "^$config_name_for_other$j" "$ShareCode_log"; then
...
export_codes_sub_only(){
    ...
    if cd "$dir_log" &>/dev/null && [[ $(ls ./*"$task_name"*/*.log 2> /dev/null | wc -l) -gt 0 ]]; then
        ...
        while IFS= read -r log_file; do
            log_files+=("$log_file")
        done < <(ls -t ./*"$task_name"*/*.log 2>/dev/null)
        pt_pin_and_code="$(SHARECODE_LABEL="$sharecode_label" perl -ne '
            my $label = $ENV{SHARECODE_LABEL};
            if (/（([^）]+)）\Q$label\E(?:】|\])\s*[:：]?\s*([^\s]+)/) {
                print "$1&$2\n";
...
case $UpdateType in
    1)
        if [[ "$ps_num" -le "$proc_num" && -n "$latest_log_path" && -f "$latest_log_path" ]]; then
            backup_del
        ...
        elif [[ -z "$latest_log_path" || ! -f "$latest_log_path" ]]; then
...
if [[ $CLEANBAK = "1" ]]; then
    echo "# 旧互助码备份清理已跳过，请手动移入回收目录后再处理。"
fi
}
...
latest_log=""
if [[ -d "$dir_code" ]]; then
    latest_log="$(ls -r "$dir_code" 2>/dev/null | head -1)"
fi
# QingLong 2.19.2 and current releases suffix task log directories with the
# numeric task ID. Fall back to an unsuffixed directory for older layouts.
newflag=true
latest_log_path=""
[[ -n "$latest_log" ]] && latest_log_path="$dir_code/$latest_log"
